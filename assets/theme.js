...
theme.Images = (function() {
  ...
  function switchImage(image, element, callback) {
    function switchSlick(variant) {
      var id = el.id,
      slideIndex = slickIndex.indexOf(id);

      if( slickIndex.indexOf(id) > -1 ){
...
slate.Variants = (function() {
   ...
   * Variant constructor
   *
    * @param {object} options - Settings from `product.json from shopify`
   */
...
theme.Search = (function() {
  ...
  function init() {
    ...
    $(selectors.siteHeaderSearchToggle).on('click', function() {
      var searchHeight = 45;
      var searchOffset = $(selectors.siteHeader).height();
...
theme.Modal = (function() {
  ...
  defaults.opener.on('click', function() {
  ...
  defaults.close.on('click', function() {
    var modalId = $(this).attr('data-id');
...
$(document).ready(function() {
  ...
  sections.register('modal', theme.Modal);
...
$(".site-header__menu").click( function(){
      $(".meganav_overlay").toggleClass('meganav_overlay_active');
});
...
// cart page input buttons
    $('.qtyplus').on('click', function(e){
        ...
        var currentVal = parseInt($(this).prev('input[field='+fieldName+']').val());

        // If is not undefined
        if (!isNaN(currentVal)) {
            // Increment
            $(this).prev('input[field='+fieldName+']').val(currentVal + 1);
        } else {
            // Otherwise put a 0 there
            $(this).prev('input[field='+fieldName+']').val(0);
        }
    });
    ...
    $(".qtyminus").on('click', function(e) {
        ...
        fieldName = $(this).attr('field');
        // Get its current value
        var currentVal = parseInt($(this).next('input[field='+fieldName+']').val());
        // If it isn't undefined or its greater than 0
        if (!isNaN(currentVal) && currentVal > 0) {
            // Decrement one
            $(this).next('input[field='+fieldName+']').val(currentVal - 1);
        } else {
            // Otherwise put a 0 there
            $(this).next('input[field='+fieldName+']').val(0);
        }
    });
});


// Read more toggle - Product page 
$(".description-more").click(function(){
    $(".description_split").toggleClass("description_hidden");
});
...
// Mobile menu button header background
$(".js-mobile-nav-toggle").click(function(){
    $(".site-header--wrapper").addClass("site-header-active-basic")
    return;
  $('.site-header--wrapper').removeClass('site-header-active-basic')
});

// Mobile search button header background
$(".site-header__search-toggle").click(function(){
    $(".site-header--wrapper").addClass("site-header-active-basic")
    return;
  $('.site-header--wrapper').removeClass('site-header-active-basic')
...
if ($bodyClass.hasClass('template-page') || $bodyClass.hasClass('template-customers-login') || $bodyClass.hasClass('template-customers-register') || $bodyClass.hasClass('template-customers-account') || $bodyClass.hasClass('template-customers-addresses') || $bodyClass.hasClass('template-cart') || $bodyClass.hasClass('template-search')) {
  $(".site-header--wrapper").addClass('site-header-active-basic');
}

$(document).ready(function() {
  $('.homepage-collection').slick({
    infinite: true,
    slidesToShow: 3,
    slidesToScroll: 1,
    dots: false,
...
if(typeof Geolizr !== "undefined") {
  Geolizr.addEventListener('currency.change', function (currency) {
    Geolizr.convertAll(Geolizr.shopCurrency, Geolizr.shopCurrency, "#cross-sell .money a, #cart .money", Geolizr.money_format)
  });
  Geolizr.domObserver({observeObject: '#cross-sell',checkForClassName: 'cross-sell'});
  Geolizr.domObserver({observeObject: '#cart',checkForClassName: 'money', observerPersists: true});
    
  Geolizr.addEventListener('currency.api', function() {
    if(location.href.startsWith("https://de.tca.fit/")) {
      Geolizr.setCurrency("EUR");
    } else {
      Geolizr.setCurrency("GPB");
    }
  });
}
